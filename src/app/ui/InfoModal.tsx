...
export default function InfoModal({ title, date, directorName, castList, genresList, overview }: InfoModalProps) {
    ...
    return (
        <>
            <div
                className="flex flex-col justify-start items-end md:flex-row space-x-0 md:space-x-4 space-y-4 md:space-y-0 h-full w-full"
            >
                <button
                    id="playButton"
                ...
                <button
                    className="text-white hover:text-gray-300 px-4 py-2 rounded flex items-center"
                >
            ...
            <div
                id="infoDialog"
                className={`fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50 space-y-0 ${open ? "flex" : "hidden"}`}
            >
                <div className="bg-slate-950 text-slate-400 p-8 rounded-lg max-w-lg">
                    ...
                    <p className="mt-4 italic text-pretty">
                        {overview}
                    </p>
                    <button
                        id="closeInfoButton"
